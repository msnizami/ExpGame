...
class UFCE():
    ...
    def barplot(self, methods, means, x_pos, serror, title, ylabel, path, save=False):
        ...
        ax.bar(x_pos, means, yerr=serror, align='center', alpha=0.5, ecolor='black', color=colors, log=False, width=0.5,
               capsize=5)
    ...
    def get_top_MI_features(self, df):
        """
        ...
        # # Calculate the mutual information between each feature pair and the target column
        # mutual_info = pd.DataFrame(index=features_df.columns, columns=features_df.columns)
        # for feature1 in features_df.columns:
        #     for feature2 in features_df.columns:
        #         if feature1 != feature2:  # Exclude pairs of the same feature
        #             mutual_info.loc[feature1, feature2] = mutual_info.loc[feature2, feature1] = mutual_info_score(
        #                 features_df[feature1], features_df[feature2])
        # # Create a list of tuples containing feature pairs and mutual information values
        # feature_pairs = []
        # for feature1 in features_df.columns:
        #     for feature2 in features_df.columns:
        #         if feature1 != feature2:  # Exclude pairs of the same feature
        #             feature_pairs.append(((feature1, feature2), mutual_info.loc[feature1, feature2]))
        # # Sort the feature pairs based on mutual information in descending order
        # feature_pairs.sort(key=lambda x: x[1], reverse=True)
        # # Extract the pairs of features from the sorted list
        # top_pairs = [pair[0] for pair in feature_pairs[:5]]  # Adjust the number 10 as needed
        # # Print the top pairs of features and their corresponding mutual information values
        # for pair in top_pairs:
        #     print(f"Features: {pair}, Mutual Information: {mutual_info.loc[pair]}")
        # return top_pairs
        ...
        # Calculate the mutual information between each feature and the target class label
        mutual_info_scores = mutual_info_classif(X, y)
        # Create a DataFrame to store the mutual information scores for each feature
        mutual_info_df = pd.DataFrame({'Feature': X.columns, 'Mutual_Information': mutual_info_scores})
        # Sort the features based on mutual information scores in descending order
        mutual_info_df_sorted = mutual_info_df.sort_values(by='Mutual_Information', ascending=False)
        # Create all possible pairs of features
        feature_pairs = list(combinations(mutual_info_df_sorted['Feature'], 2))

        # Calculate the combined mutual information scores for each feature pair
        combined_scores = []
        for pair in feature_pairs:
            combined_score = mutual_info_df_sorted[mutual_info_df_sorted['Feature'].isin(pair)][
                'Mutual_Information'].sum()
            combined_scores.append(combined_score)
        # Create a DataFrame to store the pairs and their combined scores
        combined_scores_df = pd.DataFrame({'Feature_Pair': feature_pairs, 'Combined_Score': combined_scores})
        # Sort the pairs based on their combined scores in descending order
        combined_scores_df_sorted = combined_scores_df.sort_values(by='Combined_Score', ascending=False)
        pairs = combined_scores_df_sorted['Feature_Pair'].tolist()
        # print("Pairs of Important Features (sorted by combined score):")
        # print(pairs)
        with open('modelData/feature_pairs.json', 'w') as file:
    ...
    def NNkdtree(self, data_lab1, test_inst, radius):
        ...
        idx = tree.query_ball_point(test_inst, r=radius)
        id = idx[0][0]
        # print(tree.data[id])
        nn = pd.DataFrame([tree.data[id]], columns=['Var1', 'Var2', 'Var3', 'Var4', 'Var5'])
        return nn, idx
    ...
    def Single_F(self, test_instance, u_cat_f_list, user_term_intervals, model, outcome, step, regressors):
        ...
        for feature in user_term_intervals.keys():
            if feature not in u_cat_f_list:
                ...
                pred = model.predict(tempdfcat.values)
                if pred == outcome:
    ...
    def regressionModel(self, df, f_independent, f_dependent):
        ...
        X = df.loc[:, df.columns != f_dependent]
        y = df.loc[:, df.columns == f_dependent]
    ...
    def catclassifyModel(self, df, f_independent, f_dependent):
        ...
        log_reg.fit(X_train.values, y_train.ravel())
        # Predicting the Test set results
        ...
        for f in feature_to_use_list:
            ...
            if (f1 in numf and f2 in numf) and (
                ...
                if f1 and f2 in user_term_intervals.keys():
                    ...
                    f1 in numf and f2 in u_cat_f_list) and f1 and f2 not in protected_features:  # num -> cat (binary classification)
                if f1 and f2 in user_term_intervals.keys():
                    interval_term_range1 = user_term_intervals[f1]
                    start1 = int(interval_term_range1[0])
                    end1 = int(interval_term_range1[1])
                    ...
                    if ba >= 0.8:
                        while len(f1_space) != 0:
                            ...
                            if isinstance(f2, float):
                                ...
                                temptempdf.loc[:, f2] = math.ceil(f2_val[0])
                            if math.ceil(f2_val) >= df[f2].min() and math.ceil(f2_val) <= df[f2].max():
                                two_feature_explore = pd.concat([two_feature_explore, temptempdf], ignore_index=True,
                                                                axis=0)
        ...
        for f in feature_to_use_list:
            ...
            if (f1 and f2 in numf) and (f1 and f2 not in protected_features):  # both numerical
                ...
                if f1 and f2 in user_term_intervals.keys():
                    ...
                    if mse > 1.5:
                        tempdf1 = tempdf1.loc[:, tempdf1.columns != f2]
                        f2_val = regressors[f2]["model"].predict(tempdf1.values)
                        if isinstance(f2, float):
                            temptempdf.loc[:, f2] = f2_val[0]
                        else:
    ...
    def continuous_distance(self, x, cf_list, continuous_features, metric='euclidean', X=None, agg=None):
        ...
        if metric == 'mad':
            ...
            dist = cdist(x.reshape(1, -1)[:, continuous_features], cf_list[:, continuous_features],
    ...
    def feasibility(self, X_test, cffile, X_train, features, variable_features, model, desired_outcome, label,
                    dice=False):
        cflist = cffile
        # if dice == True:
    ...
    def get_highly_correlated(self, df, features, threshold=0.5):
        ...
        correlated_features = [(corr_df.iloc[x, y], x, y) for x, y in zip(*correlated_features) if
                               x != y and x < y]  # avoid duplication
    ...
    def train_Outliers_isolation_model(self, df):
        ...
        outlier_model = IsolationForest(n_estimators=100, max_samples=1000, contamination=.05,
                                        max_features=df1.shape[1])
