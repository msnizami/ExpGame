...
class LogUserPerformanceHandler(BasisRequestHandler):
    ...
    def __parse_request_body(self):
        ...
        old_pred = self.args["oldpred"]
        if old_pred is None:
            self.send_custom_error(400, "Missing field 'plants'")
            raise Exception()

        new_pred = self.args["newpred"]
        if new_pred is None:
            self.send_custom_error(400, "Missing field 'shubNoNew'")
            raise Exception()

        return user_id, trial_count, block_count, plants, cf_plants, old_pred, new_pred

    def post(self):
        # Parse data
        try:
            user_id, trial_count, block_count, plants, cf_plants, old_pred, new_pred = self.__parse_request_body()
        except:
            return
